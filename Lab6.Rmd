...
```{r}
ggplotly(
ggplot(data = daily_report, mapping = aes(x = long, y = lat, text = region, group = group)) + 
  coord_fixed(1.3) + 
...
daily_report <-   read_csv(url("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_daily_reports/09-26-2020.csv")) %>% 
  ...
  mutate(Admin2 = tolower(Admin2))
us <- map_data("state")
md_us <- subset(us, region == "maryland")
counties <- map_data("county")
md_county <- subset(counties, region == "maryland")
state_join <- left_join(md_county, daily_report, by = c("subregion" = "Admin2")) 

ggplotly(
  ggplot(data = md_county, mapping = aes(x = long, y = lat, group = group)) + 
  coord_fixed(1.3) + 
  ...
  ggtitle("COVID-19 Cases in Maryland") +
